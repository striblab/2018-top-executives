...
function transformExecutives(data, options) {
  ...
  // Other output
  _.each(groups, (list, n) => {
    outputPrint(
      list,
      options,
      n === 'ceo' ? 50 : 10,
      n === 'ceo' ? 'ceos' : n === 'nonceo' ? 'non-ceos' : 'top-female'
    );
  });
...
function outputPrint(ceos, options = {}, limit = 50, filename = 'ceos') {
  ...
  fs.writeFileSync(
    path.join(
      options.dataOutput,
      `${new Date().toISOString().split('T')[0]}-${filename}.csv`
    ),
    csv.format(output)
  );
}
