...
function transformExecutives(data, options) {
  ...
  let groups = _.groupBy(officers, o => {
    return o.salaries[options.publishYear].title.match(/ceo/i)
      ? 'ceo'
      : 'nonceo';
  });
  ...
  _.each(groups, group => {
    ...
    group = _.sortBy(group, o => {
      return o.salaries[options.publishYear].rank;
    });
  });
...
function outputPrint(ceos, options = {}) {
  ...
  let output = ceos.map(o => {
    ...
    return {
      ...
      stock_awards: _.sum(_.filter([s.stockexpense, s.sharesvesting])) || 0,
